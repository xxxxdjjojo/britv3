...
export function SearchClient({ initialProperties, initialFilters }: SearchClientProps) {
  ...
  const [minPrice, setMinPrice] = useState(initialFilters.minPrice ?? searchParams.get("minPrice") ?? "");
  const [maxPrice, setMaxPrice] = useState(initialFilters.maxPrice ?? searchParams.get("maxPrice") ?? "");
  ...
  const [searchQuery, setSearchQuery] = useState(initialFilters.q ?? searchParams.get("q") ?? "");
  ...
  useEffect(() => {
    if (isInitialMount.current) {
      // Server already provided initial data — skip redundant fetch on first render
