...
function makeRequest(body: unknown): Request {
  return new Request("http://localhost/api/properties/prop-1/book-viewing", {
    method: "POST",
...
describe("POST /api/properties/[id]/book-viewing", () => {
  it("looks up viewing_slots by listing_id (not property_id) and books the slot", async () => {
    const slot = makeQueryBuilder({
      data: { id: "slot-1", listing_id: "prop-1", status: "available" },
      error: null,
    });
    ...
    expect(slot.eqCalls).toContainEqual(["listing_id", "prop-1"]);
