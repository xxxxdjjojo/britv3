...
export function isLocationSlug(segment: string): boolean {
  // Strict: only exact matches against the known UK location set.
  // No heuristic fallback — unknown slugs are treated as provider slugs
  // and will 404 if no provider exists with that slug.
