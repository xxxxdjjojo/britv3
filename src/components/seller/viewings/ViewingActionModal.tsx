...
const ACTION_CONFIG: Record<Action, { title: string; description: string; confirmLabel: string; confirmClass: string }> = {
  ...
  cancel: { title: "Cancel Viewing", description: "Cancel this viewing. Please provide a reason so the buyer can be informed.", confirmLabel: "Cancel Viewing", confirmClass: "bg-error text-white hover:bg-error/90" },
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
      ...
      <div role="dialog" aria-modal="true" aria-labelledby="viewing-action-title" className="relative bg-white rounded-2xl shadow-2xl w-full max-w-md p-6 z-10">
        <div className="flex items-start justify-between mb-5">
          <div>
            <h3 id="viewing-action-title" className="text-lg font-bold text-neutral-900 font-['Plus_Jakarta_Sans']">{config.title}</h3>
            <p className="text-sm text-neutral-500 mt-1">{config.description}</p>
          </div>
          <button type="button" onClick={onClose} aria-label="Close dialog" className="p-2 rounded-lg text-neutral-400 hover:bg-neutral-50 hover:text-neutral-600 transition-colors flex-shrink-0">
            <X size={18} />
          </button>
        </div>
        <div className="bg-neutral-50 rounded-xl p-4 mb-5">
          <p className="text-sm font-semibold text-neutral-600">{viewing.buyer_name}</p>
        ...
        {action === "reschedule" && (
          <div className="mb-4">
            <label className="text-sm font-semibold text-neutral-600 block mb-2">New Date & Time</label>
            <input
              type="datetime-local"
              value={newDatetime}
              onChange={(e) => setNewDatetime(e.target.value)}
              min={new Date().toISOString().slice(0, 16)}
              className="w-full px-4 py-3 rounded-xl border border-neutral-200 text-sm focus:outline-none focus:ring-2 focus:ring-brand-primary/30"
            />
          </div>
        )}
        <div className="mb-5">
          <label className="text-sm font-semibold text-neutral-600 block mb-2">
          ...
          <textarea
            ...
            className="w-full px-4 py-3 rounded-xl border border-neutral-200 text-sm resize-none focus:outline-none focus:ring-2 focus:ring-brand-primary/30"
          />
        </div>
        {error && <p className="text-error text-sm mb-4">{error}</p>}
        <div className="flex gap-3">
          <button type="button" onClick={onClose} className="flex-1 py-3 rounded-xl border border-neutral-200 text-sm font-semibold text-neutral-600 hover:bg-neutral-50 transition-colors">
            Back
          </button>
          <button
            type="button"
