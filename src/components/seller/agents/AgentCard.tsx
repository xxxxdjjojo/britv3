    ...
    <div className={cn(
      ...
      <div className="p-6">
        <div className="flex items-start gap-4">
          {agent.avatar_url ? (
            ...
            <div className="h-14 w-14 rounded-full bg-gradient-to-br from-brand-primary/20 to-[#D4A853]/20 flex items-center justify-center text-brand-primary font-bold text-xl flex-shrink-0">
        ...
        {agent.areas_covered.length > 0 && (
          <div className="mt-4 flex flex-wrap gap-1.5">
            {agent.areas_covered.slice(0, 4).map((area) => (
              <span key={area} className="flex items-center gap-1 text-xs font-medium px-2 py-0.5 rounded-full bg-muted text-slate-600">
                <MapPin size={10} />
                {area}
              </span>
            ))}
      ...
      <div className="px-6 pb-6 flex gap-2">
        <Link
          ...
          className="flex-1 py-2.5 rounded-xl bg-brand-primary text-white text-sm font-semibold text-center hover:bg-brand-primary-light transition-colors"
        >
          View Profile
        </Link>
        {onToggleCompare && (
