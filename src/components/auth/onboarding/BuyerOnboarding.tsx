...
import { X, MapPin, ArrowLeft, ArrowRight, Bell, Mail, Home, Building2, Trees, Car, Zap, Link2 } from "lucide-react";

const STEPS = ["Location", "Budget", "Preferences", "Alerts"];

type PropertyType = "Flat" | "House" | "Bungalow" | "New Build";
type MustHave = "Garden" | "Parking" | "EPC A-C" | "No chain";
...
const PROPERTY_TYPES: { value: PropertyType; icon: React.ReactNode }[] = [
  ...
  { value: "Bungalow", icon: <Home className="size-5" aria-hidden="true" /> },
  { value: "New Build", icon: <Building2 className="size-5" aria-hidden="true" /> },
];

const MUST_HAVE_OPTIONS: { value: MustHave; icon: React.ReactNode }[] = [
  { value: "Garden", icon: <Trees className="size-4" aria-hidden="true" /> },
  { value: "Parking", icon: <Car className="size-4" aria-hidden="true" /> },
  { value: "EPC A-C", icon: <Zap className="size-4" aria-hidden="true" /> },
  { value: "No chain", icon: <Link2 className="size-4" aria-hidden="true" /> },
];
    ...
    <OnboardingLayout
      ...
      subtitle={
        step === 0
          ...
          : step === 1
            ? "Set your purchase budget range. You can update this anytime."
            : step === 2
              ? "Tell us your property preferences so we can find your perfect match."
      ...
      {step === 1 && (
        <div className="space-y-6">
          ...
          <div className="space-y-2">
            <label htmlFor="min-budget" className="font-sans text-sm font-medium text-neutral-700">
              Minimum budget
            </label>
            <input
              id="min-budget"
              ...
              onChange={(e) => {
                ...
                setMinBudget(Math.min(val, maxBudget - 25000));
              }}
              aria-label="Minimum budget"
              aria-valuetext={formatGBP(minBudget)}
          ...
          {/* Max slider */}
          <div className="space-y-2">
            <label htmlFor="max-budget" className="font-sans text-sm font-medium text-neutral-700">
              Maximum budget
            </label>
            <input
              id="max-budget"
              ...
              onChange={(e) => {
                const val = Number(e.target.value);
                setMaxBudget(Math.max(val, minBudget + 25000));
              }}
              aria-label="Maximum budget"
              aria-valuetext={formatGBP(maxBudget)}
              className="w-full cursor-pointer accent-brand-primary"
            />
            <div className="flex justify-between font-sans text-xs text-neutral-400">
              <span>£50k</span>
