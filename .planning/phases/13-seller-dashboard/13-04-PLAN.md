...
must_haves:
  truths:
    ...
    - "Step 1 POST creates a new draft listing and redirects to ?step=2&id=<new_listing_id> — the ID is in the URL for all subsequent steps"
    - "Step 1: postcode lookup calls postcodes.io, populates address select, property type grid selects with visual feedback"
    - "Step 1 postcode lookup handles 404 (invalid postcode) and network errors gracefully — shows inline error message, never crashes"
    ...
    - "Step 3: upload button is disabled and shows 'Maximum 30 photos reached' when photos.length >= 30"
