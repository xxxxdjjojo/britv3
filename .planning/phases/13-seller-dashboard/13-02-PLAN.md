...
must_haves:
  truths:
    ...
    - "Analytics event recording goes through /api/analytics/event (not direct Supabase insert) with Upstash rate limiting (10 events/IP/listing/hour)"
    - "publishListing is idempotent: only updates when status != 'active' (WHERE clause prevents double-publish)"
    - "Photo count is validated server-side: PATCH for Step 3 returns 400 if photos array has > 30 items"
  artifacts:
    - path: "src/services/seller/listing-service.ts"
      provides: "CRUD for seller_listings, publish/archive transitions, listing stats joins, KPI aggregation"
    ...
    - path: "src/app/api/analytics/event/route.ts"
      provides: "POST endpoint: records a listing_analytics_events row, Upstash rate-limited at 10 events/IP/listing/hour. Replaces direct anonymous Supabase insert."
  key_links:
