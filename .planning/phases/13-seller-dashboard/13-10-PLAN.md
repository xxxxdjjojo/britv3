...
must_haves:
  truths:
    ...
    - "Find Agent page shows a grid of AgentCards filterable by area with real data from profiles table"
  ...
  artifacts:
    - path: "src/app/(protected)/dashboard/seller/valuation/page.tsx"
      provides: "Instant Valuation — postcode input, Land Registry comparables, AI estimate"
    - path: "src/app/(protected)/dashboard/seller/agents/page.tsx"
      provides: "Find an Estate Agent — area search, AgentCard grid"
    ...
    - path: "src/app/api/seller/agents/route.ts"
      provides: "GET ?area=XX — queries profiles table where role=agent"
  key_links:
