...
must_haves:
  truths:
    ...
    - "Step 7 shows a Seller Readiness Score (circular conic-gradient progress ring showing % complete) with weighted field scoring, replacing the binary checklist — sellers aim for 100% before publishing"
    - "Step 7 Publish CTA is disabled when required fields are incomplete — publish calls PATCH with action=publish which is idempotent (no-op if already active)"
  ...
  artifacts:
    ...
    - path: "src/components/seller/wizard/Step7Review.tsx"
      provides: "Listing preview card + Seller Readiness Score ring (conic-gradient, weighted fields) + micro-tips for incomplete fields + Publish CTA"
