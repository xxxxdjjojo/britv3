...
The property page's "Local area" section (`src/components/properties/detail/LocalAreaSection.tsx`, rendered from `app/(main)/properties/[slug]/page.tsx`) composes independent, source-attributed data layers. Each layer self-gates: its widget renders only when real data exists, so the section degrades gracefully. One layer (crime) calls a live API at render (Redis-cached); flood calls a live WMS; the rest are DB-backed (read from tables populated by ingest scripts).
- **Schools** — `schools` (PostGIS: GIAS establishments, Open + real phase, with the legacy Ofsted overall-effectiveness grade where one still exists) + `get_nearby_schools` RPC; `ofsted-service.ts` → `SchoolCatchmentWidget` (rating badge shown only when a grade exists). Ingest: `scripts/ingest-gias-schools.mjs` (DfE GIAS bulk + Ofsted "latest inspections" MI, OGL v3.0). NOTE: GIAS bulk carries no Ofsted grade and post-Sep-2024 inspections dropped the single overall grade — hence ~45% of schools have a rating and the rest render name/phase/distance only.
- **Crime** — live API: data.police.uk (`crime-service.ts`). OGL v3.0.
