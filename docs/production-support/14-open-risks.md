...
| OR-09 | Logical restore is **evidenced** (2026-07-14 drill: 206 business tables, 192,142 rows restored identically — `scripts/dr/evidence/`); but Supabase PITR *tier/window* and Storage-bucket (non-Postgres) backup coverage remain unverified, and no PITR-to-branch RTO has been measured | Logical restore proves the data path cheaply; PITR/Storage need dashboard access this session lacks | Executed restore drill + reproducible procedure; documented in `12-dr-and-backups.md` | Before scaling / funding round; verify PITR + run one PITR restore |
