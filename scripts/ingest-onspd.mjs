...
function buildColMap(headers) {
  ...
  const pcdsCandidates = ["pcds", "pcd2", "pcd"];
  let pcdsCol = -1;
  for (const c of pcdsCandidates) {
    const i = idx(c);
    if (i !== -1) { pcdsCol = i; break; }
  }

  // Prefer 2021 census codes, fall back to 2011
  const oaCol   = idx("oa21")   !== -1 ? idx("oa21")   : idx("oa11");
  const lsoaCol = idx("lsoa21") !== -1 ? idx("lsoa21") : idx("lsoa11");
  const msoaCol = idx("msoa21") !== -1 ? idx("msoa21") : idx("msoa11");
...
async function main() {
  ...
  for await (const rawLine of rl) {
    ...
    if (colMap === null) {
      ...
      if (colMap.lat === -1 || colMap.long === -1) {
        console.error(
          "[onspd] ERROR: Could not find lat/long columns in CSV header.\n" +
          `         Detected headers (first 10): ${headers.slice(0, 10).join(", ")}\n`,
        );
        if (client) await client.end();
        process.exit(1);
      ...
      console.log(
        `[onspd] Header mapped — postcode col idx: ${colMap.pcds}, ` +
    ...
    // --- coordinates (ONSPD sentinel: lat=99.999999 means no coords) ---
